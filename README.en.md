...
- **`webapp/StradarioViewer/`** — a pocket companion: Blazor WebAssembly
  (.NET 8), 100% client-side (no backend/database), installable as a PWA.
  Loads a `.stradario` file (kept in the browser's `localStorage` for later
  visits) and lets you browse dated routes/POI day by day on a map, with
  each item's label and description. Independent .NET project in its own
  repo subfolder, not referenced by the desktop project — see
  `webapp/CLAUDE.md`.
