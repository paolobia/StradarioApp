...
- **Optional date/time on POI and routes**: an optional From/To field on every POI and route — when set, the navigation tree reorders chronologically automatically and the PDF gets a dedicated "Itinerary" page (undated items last) right after the cover; with no date set anywhere, the PDF stays identical to before (no extra page); in the PDF the date is prefixed with the weekday abbreviation (**Sunday in bold**), the start/end time is omitted when it's 00:00/23:59, and an event spanning the whole day (00:00 → 23:59 same day) doesn't produce a separate "End" row
