...
- **Pages**: right-click to add, drag to move, manual/automatic lock against accidental moves, automatic labels (A1, B2…), drag-to-reorder in the side tree, **per-page orientation and scale override** (independent of the project's general settings)
- **POI groups**: markers with customizable icon/color, added directly on the map or via search — 43 built-in categories plus custom ones, live search (Overpass, with an offline local match score always computed alongside the optional AI/Groq filter) or offline (downloadable per-continent local database), address and city search; move a POI between groups with a cut/paste gesture; drag-to-reorder POI within their own group (affects PDF order); a hidden group (eye off) is protected from accidental changes; a group's color auto-syncs with a coincident route's color
- **Routes**: point-by-point drawing on the map (Enter or shift+click to confirm, panning while drawing no longer adds a stray point), extendable and editable afterwards, routable onto real roads via OSRM (driving/cycling/walking, multiple alternatives selectable directly on the map); its label auto-shifts away when too close to a POI
- **Universal import/export**: a single command imports KMZ/KML/GPX (POI and routes together, with distinct colors for imported POI groups); export either separate, combined, or a single group/route; also **CSV** (two tabular files, one for POI and one for routes, that open cleanly in Excel/LibreOffice); names in a non-Latin script are cleaned up automatically (including in the "What's here" tooltip); points in China (possibly GCJ-02 instead of WGS84) handled with automatic or manual correction
- **PDF generation**: preview before saving, **cover page with title**, index, overview map, pages with references to adjacent ones and a graphic scale bar, real word-wrap for long descriptions, a POI group is never split from its own list by a page break; 5 print contrast modes for the map (none, color, black/white, road emphasis, **adaptive/local**), plus **edge reinforcement** and **B/W print dithering** as independent options
