...
## Features
1. **Settings**: three tabs. "Generale" (General): page format (A5/A4/A3), orientation,
   ...
   built-in ones, persisted globally. "Database POI offline" (Offline POI
   database): optional per-continent download (`PoiOfflineDatabase`, data
   generated by `osm/OsmExtractor` from Geofabrik extracts) for instant,
   network-free search instead of live Overpass
...
7. **Unified import/export**: a single toolbar button imports KMZ/KML/GPX
   (POI and routes in the same file, automatic merge); export either
   separate (POI groups, routes) or combined into a single file ("Export
   all", `BuildCombinedKmlDocument` for KML/KMZ, a shared GPX root for
   GPX). Names/tags in a non-Latin script are cleaned up/replaced with an
   ASCII variant (`Services/AsciiText`). Points in China may be in GCJ-02
   rather than real WGS84 (`GcjTransform`, an offset also used by Google
   Maps for locations inside China): for KML/KMZ never asked (always WGS84
   by specification), for GPX an explicit choice (or auto-detected from the
   file name) defaulting to "already WGS84"; C→W/W→C icons in the tree let
   you manually correct/convert any single point in the China area,
   regardless of how it was created
8. **Hybrid POI search**: live Overpass by default, local offline database
   (if downloaded) as an instant network-free alternative
9. **PDF generation**: preview (temp file → system viewer → Save/Close
...
10. **Project saving**: `.stradario` file (JSON) with all
    ...
    in `AppPreferencesService` (global user preferences). Every file picker
    (open/save project, import, export, PDF) starts from the last folder
    actually used (`AppPreferencesService.LastUsedFolder`), not "Recent"
